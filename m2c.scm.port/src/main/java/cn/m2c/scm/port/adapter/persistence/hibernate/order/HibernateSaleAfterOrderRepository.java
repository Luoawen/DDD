...
public class HibernateSaleAfterOrderRepository extends HibernateSupperRepository implements SaleAfterOrderRepository {
	...
	/***
	 * 获取商家已同意退款的售后订单，但还未退款
	 */
	@Override
	public List<SaleAfterOrder> getReturnMoneyOrder(int minute) {
		return (List<SaleAfterOrder>)this.session().createSQLQuery("SELECT * FROM t_scm_order_after_sell WHERE _status=7 AND order_type=0 AND round((UNIX_TIMESTAMP(now())-UNIX_TIMESTAMP(last_updated_date))/60) > " + minute)//60/
				.addEntity(SaleAfterOrder.class).list();
	}
}
